...
(async () => {
  ...
  /* Sem esta guarda, uma janela que abriu SEM o preload (o preload-recorte.js fora do
     build.files, por exemplo) estouraria aqui e ficaria preta por cima de tudo, com o
     Cockpit escondido atras e nenhuma tecla funcionando. Assim ela ao menos se explica —
     e o vigia de 15 s do main fecha sozinho. */
  const api = window.recorte || null;
  if (!api) { dica.textContent = 'Esta janela abriu sem a ponte do recorte. Ela se fecha sozinha em alguns segundos.'; return; }
  let dados = null;
  try { dados = await api.dados(); } catch {}
  if (!dados || !dados.png) { dica.textContent = 'Não consegui carregar a foto da tela.'; setTimeout(() => { try { api.cancelar(); } catch {} }, 1200); return; }
  foto.src = dados.png;
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { e.preventDefault(); try { api.cancelar(); } catch {} }
    if (e.key === 'Enter') { e.preventDefault(); confirmar(); }
  });
  document.addEventListener('contextmenu', (e) => { e.preventDefault(); try { api.cancelar(); } catch {} });
})();
